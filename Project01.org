...
*** TODO Checker Implementation - ELISP
...
def passwordChecker(password):
    ...
    # Number of passwords a modern computer can compute per second 
    checksPerSec = 588235 
    
    # Update the number of of each type of character
    for c in password:
        if c.isupper():
            upper = 26
        elif c.islower():
    ...
    # Get total number of characters in our set
    total = lower + upper + digit + symbol

    # Get the number of possible permutations
    numPermutation = total ** len(password)
    
    # Calculate seconds to brute force password in the worst case
    seconds = (numPermutation / checksPerSec)

    # Seconds if password is brute forced in half the time
    halfSeconds = seconds / 2

    secondsPerYear = 525600 * 60
    
    years = halfSeconds / secondsPerYear
    
    print("Number of years to brute force password:", years)
...
#+RESULTS:
: Number of years to brute force password: 0.005628586175835452
